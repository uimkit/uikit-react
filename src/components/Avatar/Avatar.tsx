...
export function Avatar<T extends AvatarProps>(props:T):React.ReactElement {
  ...
  return (
    <div
      className={`uim-kit-avatar ${shape}`}
      style={{
        flexBasis: `${size}px`,
        height: `${size}px`,
        objectFit: 'cover',
    ...
    >
      ...
      {
        update && (
          <div className={`uim-kit-avatar-edit ${isEdit ? 'displayFlex' : ''}`} role="button" tabIndex={0} onClick={toggle}>
          ...
          >
            {
              list.map((item:string, index:number) => {
                ...
                return (
                  <li
                    className="uim-kit-avatar-list-item"
                    role="menuitem"
                    key={key}
                    tabIndex={index}
