 ...
 *
 * ```
 * const i18n = new Uimi18n({ language 'nl' });
...
export class Uimi18n {
  ...
  constructor(options: Options = {}) {
    ...
    try {
      ...
      throw Error(
        `Uimi18n: Looks like you wanted to provide Dayjs instance, but something went wrong while adding plugins ${error}`,
      );
    }
    ...
    this.i18nextConfig = {
      ...
      parseMissingKeyHandler: (key) => {
        this.logger(`Uimi18n: Missing translation for key: ${key}`);
    ...
    if (dayjsLocaleConfigForLanguage) {
      ...
      this.logger(
        `Uimi18n: Uimi18n(...) - Locale config for ${this.currentLanguage} does not exist in momentjs.` +
          `Please import the locale file using "import 'moment/locale/${this.currentLanguage}';" in your app or ` +
          `register the locale config with Uimi18n using registerTranslation(language, translation, customDayjsLocale)`,
      );
    }
  ...
  validateCurrentLanguage = () => {
    ...
    if (availableLanguages.indexOf(this.currentLanguage) === -1) {
      this.logger(
        `Uimi18n: '${this.currentLanguage}' language is not registered.` +
          ` Please make sure to call uimi18n.registerTranslation('${this.currentLanguage}', {...}) or ` +
  ...
  registerTranslation(
    ...
    if (!translation) {
      this.logger(
        `Uimi18n: registerTranslation(language, translation, customDayjsLocale) called without translation`,
      );
      return;
    }
    ...
    if (customDayjsLocale) {
      ...
      this.logger(
        `Uimi18n: registerTranslation(language, translation, customDayjsLocale) - ` +
